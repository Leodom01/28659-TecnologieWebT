...
public class PriceRequest extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		mySession.setAttribute("day1", date1);
		mySession.setAttribute("day2", date2);
		mySession.setAttribute("id", id);
		
		
		DatiReader myReader = (DatiReader) this.getServletContext().getAttribute("datiReader");
		
		int staticPrice = myReader.getPrezzo().get(id);
		
		int finalPrice = staticPrice*(date2-date1);
		
		req.setAttribute("price", finalPrice);
