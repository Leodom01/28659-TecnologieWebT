...
function parsificaXmlJSON( xml ) {
	...
	for (    var a = 0, b = items.length;    a < b;   a++   ) {
		// [length al posto di push serve per evitare errori con vecchi browser]
		itemNodes[a] = JSON.parse(items[a].firstChild.nodeValue);
	}// for ( items )
	...
	theXhr.onreadystatechange = function() { 
		...
		if ( theXhr.readyState === 2 ) {
			...
			if ( theXhr.status === 200 ) {
				// operazione avvenuta con successo
				if ( theXhr.responseXML ) {
					theElement.innerHTML = parsificaXmlJSON(theXhr.responseXML);
...
function fillIfDoable(formInput){
	var xhr = myGetXmlHttpRequest();
	
	xhr.onreadystatechange = function(){
		// verifica dello stato
		if ( xhr.readyState === 2 ) {
	    	formInput.innerHTML = "Richiesta inviata...";
		}// if 2
		else if ( xhr.readyState === 3 ) {
    		formInput.innerHTML = "Ricezione della risposta...";
		}// if 3
		else if ( xhr.readyState === 4 ) {
			// verifica della risposta da parte del server
			if ( xhr.status === 200 ) {
				// operazione avvenuta con successo
				if(xhr.responseXML.getElementsByTagName("inferredString").length != 0){
					formInput.value = xhr.responseXML.getElementsByTagName("inferredString").item(0).firstChild.nodeValue;
				}else{
					console.log("Nessun match trovato!");
				}
							
			}else{
				console.error("Errore nella xhr!");
			}
		}
	}
	
	try {
		xhr.open("get", "contents/feed.jsp?fill="+formInput.value, true);
	}
	catch(e) {
		// Exceptions are raised when trying to access cross-domain URIs 
		alert(e);
