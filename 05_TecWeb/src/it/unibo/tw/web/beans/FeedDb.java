...
public class FeedDb implements Serializable {
	...
	public FeedDb() {
		...
		this.feeds.add(feed);
		this.categroies.add(feed.getCategory());
		...
		feed = new Feed(
				"Ciccio Gamer vince il pallone oro",
				"Tutti se lo aspettavano, ma risulta sempre una sorpresa",
				"",
				"SPORT",
				"Mon, 16 Feb 2009 16:58:35 +0200",
				"http://www.lastampa.it/_web/cmstp/tmplrubriche/sport/Scherzone!!!",
				""
			);
		this.feeds.add(feed);
		this.categroies.add(feed.getCategory());
	...
	public String getCategoriesMatching(String input){
		for(String currentCat : this.categroies) {
			if(currentCat.startsWith(input)) {
				return currentCat;
			}
		}
		return null;
