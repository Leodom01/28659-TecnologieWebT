...
function sendData(inputField, item){
	if((inputField.value[0] >= "A" && inputField.value[0] <= "Z")){
	...
	xhr1.onreadystatechange = function(){
		if ( xhr1.readyState <= 2 ) {
		    	console.log("xhr1: Richiesta inviata...");
		}else if ( xhr1.readyState === 3 ) {
	    		console.log("xhr1: Ricezione della risposta...");
		}else if ( xhr1.readyState === 4 ) {
			if ( xhr1.status === 200 ) {
				console.log("xhr1: Ho la risposta..."+xhr1.responseText);
				writeResult(xhr1, xhr2, item)
				}
			}else {
				console.error("Errore, riparti");
			}
		}
	xhr2.onreadystatechange = function(){
		if ( xhr2.readyState <= 2 ) {
		    	console.log("xhr2: Richiesta inviata...");
		}else if ( xhr2.readyState === 3 ) {
	    		console.log("xhr2: Ricezione della risposta...");
		}else if ( xhr2.readyState === 4 ) {
			if ( xhr2.status === 200 ) {
				console.log("xhr2: Ho la risposta..."+xhr2.responseText);
				writeResult(xhr1, xhr2, item)
				}
			}else {
				console.error("Errore, riparti");
	...
	xhr1.open("get", "./Counter1?car="+inputField.value, true);
	xhr2.open("get", "./Counter2?car="+inputField.value, true);	
...
function writeResult(xhr1, xhr2, item){
	if(xhr1.readyState == 4 && xhr2.readyState == 4){
		item.innerHTML = eval(xhr1.responseText)+eval(xhr2.responseText);
