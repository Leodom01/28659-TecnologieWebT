...
public class Register extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if(utenti.getUtenteByName(username) == null && utenti.allowedGroup.contains(group)) {
			//New registration
			...
			rd.forward(req, res);
			return;
		}else if(utenti.getUtenteByName(username) == null && !utenti.allowedGroup.contains(group)) {
			//Non existent group 
			req.setAttribute("registerOutcome", "invalidGroup");
			RequestDispatcher rd = this.getServletContext().getRequestDispatcher("/loginPage.jsp");
			rd.forward(req, res);
			return;
		}else if(((testUser = utenti.getUtenteByName(username)) != null && testUser.password.equals(oldPassword) && testUser.gruppo.equals(group) && testUser.isStillValid())){
